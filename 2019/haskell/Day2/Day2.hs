...
part1 x = case Cpu.executeNoInput $ Cpu.primeTape [(1,12),(2,2)] $ Cpu.parseInput x of
  ...
  Right state -> show $ head $ toList $ Cpu.mem $ Cpu.tapeState $ state

part2 :: String -> String
part2 x = head (((map (\(p,res) -> ((show p) ++ " -> " ++ show (100*(fst p) + (snd p))))) . 
             (filter (\(p,res) -> either (\_ -> False) (\t -> ((Seq.index (Cpu.mem $ Cpu.tapeState t) 0) == 19690720)) res)) .
